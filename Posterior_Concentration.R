...
# write_xlsx(sampleProbList, "sampleProb.xlsx")
# write_xlsx(as.data.frame(Pi.error.mat), "RMSE of HSBP and True.xlsx")
...
plot(sample.vec, distances1, type="b", pch=19, col="blue", lwd=2,
     xlab="Sample Size", ylab="Root Mean Squared Error",
     ylim=range(c(distances1, distances2, distances3)),
     main="")

lines(sample.vec, distances2, type="b", pch=17, col="red", lwd=2)
lines(sample.vec, distances3, type="b", pch=17, col="green", lwd=2)
legend("topright",
       legend=c("MLE", "HSBP", "GHSBP"), # Update names as needed
...
############################### Rate of Convergence ################################
library(readxl)
distancesData <- read_excel("Distances_PostConc.xlsx", sheet = 1)
